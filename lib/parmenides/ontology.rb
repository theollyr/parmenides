...
module Parmenides
  class Ontology
    ...
    def klass(label)
      label = label.to_s.split("/").last if label.is_a?(::RDF::URI)
  ...
  class StaticOntology
    class Klass
      ...
      def inspect
        "#<#{self.class}:#{object_id} #{uri.to_s.split("/").last}>"
      end
      alias_method :to_s, :inspect

      def descendants_count(limit_level = nil)
        return 0 if !limit_level.nil? && level > limit_level
    ...
    def load_klasses
      ...
      results.each do |result|
        if result.all? { |_, v| ::RDF::OWL.Thing == v || v =~ %r{^http://dbpedia.org/ontology} }
