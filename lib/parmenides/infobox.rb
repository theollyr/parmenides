module Parmenides
  class Infobox
    ...
    def inspect
      "#<%s:%d URI:%s>" % [self.class.to_s, object_id, uri]
    end

    def uri
      @uri ||= environment.main.resource.vocabulary.__send__(
        "#{environment.main.template}#{name}")
    end
    ...
    def resources
      @resources ||= begin
        ...
        environment.client.instance.query(resource_query).each do |sol|
          ...
          intres.type << environment.ontology.instance.klass(
            sol[:type].to_s.split("/").last)
        end
    ...
    def resource_query
      @resource_query ||= begin
        ...
        temp << environment.other.uris.map(&:to_base).join(" ")
    ...
    def properties
      @properties ||= begin
        ...
        0.step(to: count - 1, by: step) do |offset|
          ...
          resources = environment.client.instance.query(
            resource_query.sub("#offset#", offset.to_s))
    ...
    def resources_to_cache
      YAML.dump(resources.map do |res|
        ...
        oths = res.same_as.map do |ore|
          ouri = ore.uri.to_s
          type = ore.type.max_by(&:level).uri.to_s
